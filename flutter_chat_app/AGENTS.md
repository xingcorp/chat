...
- **NO Flutter imports** — pure Dart only (`dartz`, `equatable` OK)
- **NO Data layer imports** — `import 'package:flutter_chat_app/data/...'` is FORBIDDEN
- **NO Presentation layer imports**
- Repository interfaces MUST use `I` prefix (`IMessageRepository`)
- All repository methods MUST return `Either<Failure, T>`
- Use cases MUST extend `UseCase<ReturnType, Params>`
- Entities MUST be immutable (`const` constructors, `final` fields)

## Data Layer Rules

- DTOs: `@freezed` + `@JsonSerializable` with `@JsonKey(name: 'backendField')` for mapping
- Repos: `@LazySingleton(as: IRepository)`, follow offline-first pattern, use `RepositoryErrorMixin`
- Mappers: `toEntity()` / `fromEntity()`, handle null fields gracefully
- Remote datasources: GraphQL via `graphql_client.dart`
- Local datasources: Isar for offline storage
- NEVER import Presentation layer

## Presentation Layer Rules

- BLoCs: `@injectable`, extend `BaseBloc`, dispose subscriptions in `close()`
- States: `@freezed` + extend `BaseState`
- Widgets: `BaseStatefulWidget`/`BaseStatelessWidget`, use `safeSetState()`
- All strings via `context.l10n`, colors via `AppColors`, dimensions via `AppDimens`
