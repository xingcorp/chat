...
/// Extension methods for [MessageStatus]
extension MessageStatusExtension on MessageStatus {
  /// Returns true if the message is in a final state (not sending/pending)
  bool get isFinal => this != MessageStatus.sending && 
  ...
  /// Returns true if the message needs retry
  bool get needsRetry => this == MessageStatus.failed;

  /// Returns the number of checkmarks to display
  int get checkmarkCount {
    switch (this) {
...
/// Extension methods for [ReactionType]
extension ReactionTypeExtension on ReactionType {
  /// Returns the emoji string for the reaction
  String get emoji {
    switch (this) {
      case ReactionType.thumbsUp:
        return '👍';
  ...
  /// Returns the accessibility label for the reaction
  String get accessibilityLabel {
    switch (this) {
      case ReactionType.thumbsUp:
        return 'Thumbs up';
...
extension AudioPlaybackSpeedExtension on AudioPlaybackSpeed {
  ...
  /// Returns the display label for the speed
  String get label {
    switch (this) {
      case AudioPlaybackSpeed.slow:
        return '0.5x';
