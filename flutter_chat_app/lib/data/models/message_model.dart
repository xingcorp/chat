...
class MessageModel {
  ...
  /// **Create MessageModel from Backend API Response**
  ///
  /// Maps backend message object to MessageModel.
  /// Backend uses different field names and structure.
  ///
  /// **Backend Fields:**
  /// - id, message (not content), urls[], type
  /// - createdAt, editAt, deletedAt
  /// - senderId, sender { id, fullname, avatarUrl }
  /// - conversationId (not chatId), readerIds (not readBy)
  /// - reactions[], replyMessageId, forwardedFromMessageId, fileName
  ///
  /// **Parameters:**
  /// - map: Backend message object
  ///
  /// **Returns:** MessageModel instance
  factory MessageModel.fromBackendMap(Map<String, dynamic> map) {
    // Generate local ID from server ID or create new
    final serverId = map['id'] as String?;
    final localId = serverId ?? const Uuid().v4();
    
    // Parse type
    final typeStr = (map['type'] as String?)?.toLowerCase() ?? 'text';
    ...
    final updatedAt = _parseTimestamp(map['editAt']);
    
    // Check if deleted
    final isDeleted = map['deletedAt'] != null;
    
    // Extract readBy from readerIds
    final readBy = (map['readerIds'] as List?)
        ?.map((id) => id.toString())
        .toList() ?? <String>[];
    ...
    return MessageModel(
      serverId: serverId,
      localId: localId,
      chatId: map['conversationId'] as String? ?? '',
      senderId: map['senderId'] as String? ?? '',
      readBy: readBy,
      content: map['message'] as String? ?? '', // Backend uses 'message'
      type: type,
      status: MessageStatus.sent, // Backend messages are already sent
      createdAt: createdAt,
      updatedAt: updatedAt,
      replyToMessageId: map['replyMessageId'] as String?,
      metadata: metadata,
      isDeleted: isDeleted,
  ...
  /// **Convert MessageModel to Backend API Format**
  ///
  /// Converts MessageModel to format expected by backend mutations.
  /// Used for sending messages.
  ///
  /// **Backend Expected Format:**
  /// - conversationId: String (not chatId)
  /// - receiverId: String (optional, for direct chat)
  /// - type: String (uppercase: "TEXT", "IMAGE", etc.)
  /// - message: String (not content)
  /// - urls: [String] (extracted from metadata)
  /// - fileName: String (extracted from metadata)
  /// - replyMessageId: String
  /// - forwardedFromMessageId: String (extracted from metadata)
  /// - createdAt: Float (milliseconds since epoch)
  ///
  /// **Parameters:**
  /// - receiverId: Optional receiver ID for direct chat
  ///
  /// **Returns:** Map ready for backend API
  Map<String, dynamic> toBackendMap({String? receiverId}) {
    ...
    final result = <String, dynamic>{
      'type': type.name.toUpperCase(), // Backend expects uppercase
      'message': content, // Backend uses 'message' not 'content'
      'createdAt': createdAt.millisecondsSinceEpoch, // Backend expects timestamp
    };
    
    // Add conversationId or receiverId (mutually exclusive)
    if (receiverId != null) {
      result['receiverId'] = receiverId;
    } else {
      result['conversationId'] = chatId;
    }
    ...
    if (replyToMessageId != null) {
      result['replyMessageId'] = replyToMessageId;
    }
