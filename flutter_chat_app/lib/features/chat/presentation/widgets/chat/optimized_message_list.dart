...
class _OptimizedMessageListState extends State<OptimizedMessageList>
  ...
  void didUpdateWidget(OptimizedMessageList oldWidget) {
    ...
    if (widget.messages.length > oldWidget.messages.length) {
      final newMessages = widget.messages
          .take(widget.messages.length - oldWidget.messages.length);
      final hasNewMessagesFromCurrentUser =
          newMessages.any((m) => m.sender.id == widget.currentUserId);
  ...
  void scrollToIndex(int index, {bool animated = true}) {
    ...
    _performanceMonitor.startTrace(TraceType.custom,
        customTraceName: 'scroll_to_message');
    _scrollController
        .scrollToIndex(
      index,
      ...
      duration: animated ? _scrollDuration : Duration.zero,
        ...
        .then((_) {
      _performanceMonitor.stopTrace(TraceType.custom,
          customTraceName: 'scroll_to_message');
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget messageList = ListView.builder(
      ...
      cacheExtent: MediaQuery.of(context).size.height *
          2, // Increase cache for smoother scrolling
