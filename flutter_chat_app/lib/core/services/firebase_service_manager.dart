...
class FirebaseServiceManager {
  ...
  Future<void> initializeServices() async {
    if (!FirebaseConfigManager.supportsConfiguredPlatform) {
      _logger.i(
        'Skipping Firebase services on '
    ...
    try {
      ...
      _logger.e('Failed to initialize Firebase services',
  ...
  Future<void> _initializeAnalytics(FlavorConfig config) async {
    ...
    try {
      ...
      _logger.e('Failed to initialize Analytics',
          error: e, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> _initializeCrashlytics(FlavorConfig config) async {
    ...
    try {
      ...
      _logger.e('Failed to initialize Crashlytics',
          error: e, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> _initializePerformanceMonitoring(FlavorConfig config) async {
    ...
    try {
      ...
      _logger.i(
          'Firebase Performance Monitoring initialized for ${config.flavor.name}');
    } catch (e, stackTrace) {
      _logger.e('Failed to initialize Performance Monitoring',
          error: e, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> _initializeMessaging(FlavorConfig config) async {
    try {
      ...
      //   '${config.isProduction ? 'prod' : 'staging'}_general',
      // );
      // await messaging.subscribeToTopic(
      //   '${config.isProduction ? 'prod' : 'staging'}_updates',
      // );

      _messagingInitialized = true;
      _logger.i('Firebase Messaging initialized for ${config.flavor.name}');
    } catch (e, stackTrace) {
      _logger.e('Failed to initialize Messaging',
          error: e, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> logEvent(String name, Map<String, dynamic>? parameters) async {
    ...
    try {
      ...
      //   parameters: {
      //     ...?parameters,
      //     'app_flavor': FlavorConfig.instance.flavor.name,
      ...
      _logger.e('Failed to log analytics event',
          error: e, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> recordError(
    ...
    try {
      ...
      _logger.e('Failed to record error to Crashlytics',
          error: e, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> startTrace(String name) async {
    ...
    try {
      ...
      _logger.e('Failed to start performance trace',
          error: e, stackTrace: stackTrace);
    }
  }
    ...
    return {
      ...
      'environment': {
        ...
        'enablePerformanceMonitoring':
            config.environment.enablePerformanceMonitoring,
      },
    };
  }
}
