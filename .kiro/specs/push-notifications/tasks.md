...
- [ ] 1. Thêm GraphQL operation
  - [ ] 1.1 Thêm `notifyUser` mutation vào `ChatMutations` trong `flutter_chat_app/lib/data/graphql/chat_operations.dart`
    - Mutation `chatNotifyUser(arguments: ChatNotifyUserData!)` trả về `Boolean`
    - _Requirements: 1.1, 1.2, 1.3_

- [ ] 2. Tạo Data layer
  - [ ] 2.1 Tạo `PushNotificationRemoteDataSource` tại `flutter_chat_app/lib/data/datasources/notification/push_notification_remote_datasource.dart`
    - `@lazySingleton`, inject `GraphQLClientWrapper`
    - `notifyUsers(receiverIds, title, content, metadata)` — gọi GraphQL mutation
    - _Requirements: 2.2_
  - [ ] 2.2 Tạo `PushNotificationRepositoryImpl` tại `flutter_chat_app/lib/data/repositories/push_notification_repository_impl.dart`
    - `@LazySingleton(as: IPushNotificationRepository)`
    - Wrap trong try/catch, return `Either<Failure, bool>`
    - _Requirements: 2.4_

- [ ] 3. Tạo Domain layer
  - [ ] 3.1 Tạo `IPushNotificationRepository` interface tại `flutter_chat_app/lib/domain/repositories/i_push_notification_repository.dart`
    - `notifyUsers(receiverIds, title, content, metadata)` → `Either<Failure, bool>`
    - _Requirements: 2.1_
  - [ ] 3.2 Tạo `SendPushNotificationUseCase` tại `flutter_chat_app/lib/domain/usecases/notification/send_push_notification_usecase.dart`
...
- [ ] 4. Tích hợp vào MessageBloc
  - [ ] 4.1 Inject `SendPushNotificationUseCase` vào `MessageBloc`
  - [ ] 4.2 Sau khi send message thành công có mentions, gọi `sendPushNotification`
...
- [ ] 5. Cập nhật Deep Link Handling
  - [ ] 5.1 Cập nhật notification tap handler hiện có để parse `metadata.conversationId`
  - [ ] 5.2 Navigate đến `ChatDetailsPage` khi tap notification có conversationId
  - [ ] 5.3 Xử lý trường hợp conversation không tồn tại — hiển thị `AppSnackBar`
