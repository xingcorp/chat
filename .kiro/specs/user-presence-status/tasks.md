...
- [x] 2. Tạo Domain layer
  - [x] 2.1 Tạo `UserPresence` entity tại `flutter_chat_app/lib/domain/entities/user_presence.dart`
    - Fields: `userId`, `isOnline`, `lastSeen?`
    - Static `offline` factory
    - _Requirements: 1.4_
  - [x] 2.2 Tạo `IPresenceRepository` interface tại `flutter_chat_app/lib/domain/repositories/i_presence_repository.dart`
    - `getUsersPresence(List<String> userIds)` → `Either<Failure, List<UserPresence>>`
    - _Requirements: 1.1, 1.3_

- [x] 3. Tạo Data layer
  - [x] 3.1 Tạo `PresenceQueries` GraphQL operations tại `flutter_chat_app/lib/data/graphql/presence_operations.dart`
    - Query `getUserPresence` với `userIds` parameter
    - _Requirements: 1.1_
  - [x] 3.2 Tạo `UserPresenceModel` DTO tại `flutter_chat_app/lib/data/models/user_presence_model.dart`
    - `fromJson` factory, `toEntity()` mapper
  - [x] 3.3 Tạo `PresenceRemoteDataSource` tại `flutter_chat_app/lib/data/datasources/presence/presence_remote_datasource.dart`
    - `@lazySingleton`, inject `GraphQLClientWrapper`
    - `getUsersPresence(userIds)` — gọi GraphQL query
    - _Requirements: 1.1, 1.3_
  - [x] 3.4 Tạo `PresenceRepositoryImpl` tại `flutter_chat_app/lib/data/repositories/presence_repository_impl.dart`
    - `@LazySingleton(as: IPresenceRepository)`
    - Wrap trong try/catch, return `Either`
    - _Requirements: 1.1_

- [x] 4. Tạo `PresenceService` (Core)
  - [x] 4.1 Tạo `PresenceService` tại `flutter_chat_app/lib/core/services/presence_service.dart`
...
- [x] 5. Mở rộng `RealtimeService` — thêm presence stream
  - [x] 5.1 Thêm `presenceStream` vào `RealtimeService` (hoặc `EnhancedSocketManager`)
    - Subscribe Socket.IO event `user:presence`
    - Parse thành `UserPresence` object
    - _Requirements: 2.1, 2.2_

- [x] 6. Checkpoint — Verify service layer hoạt động

- [x] 7. Cập nhật Presentation widgets
  - [x] 7.1 Cập nhật `LivePresenceIndicator` tại `flutter_chat_app/lib/features/chat/presentation/widgets/chat/presence_indicator.dart`
  ...
  - [x] 7.2 Cập nhật `PresenceIndicator` và `AvatarPresenceIndicator` sử dụng `AppColors` thay vì `Colors.green`/`Colors.grey`
    - _Requirements: 3.3_

- [x] 8. Tích hợp vào Chat List
  - [x] 8.1 Trong chat list page, gọi `presenceService.fetchPresenceForUsers(memberIds)` khi load danh sách
  - [x] 8.2 Trong chat list item (direct chat), wrap avatar với `AvatarPresenceIndicator` kết nối `PresenceService`
    - Chỉ hiển thị cho direct chat
    - _Requirements: 3.1, 3.2, 3.4_

- [x] 9. Tích hợp vào Chat Detail Header
  - [x] 9.1 Trong chat detail header (direct chat), thêm `LivePresenceIndicator(userId: otherUserId)` dưới tên user
    - Hiển thị "Online" hoặc "Lần cuối X trước"
    - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5_

- [x] 10. DI Registration và Code Generation
  - [x] 10.1 Đảm bảo `PresenceService`, `PresenceRemoteDataSource`, `PresenceRepositoryImpl` đăng ký trong DI
  - [x] 10.2 Chạy `dart run build_runner build --delete-conflicting-outputs`
