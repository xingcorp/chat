...
@Injectable()
export class IsExistTagDocumentDbValidateConstraint extends BaseValidateDecoratorConstraint {

    constructor(private readonly tagDocumentRepo: TagDocumentRepo) {
        super()
        this.messageKey = 'TagDocumentNotFound'
    }
    ...
    async checkValidate(ids: string | string[], args: ValidationArguments) {
        ...
        const list = await this.tagDocumentRepo.getBy(where)
...
export function IsExistTagDocumentDbValidate(validationOptions?: ValidationOptions) {
    return function (object: Object, propertyName: string) {
        baseRegisterDecorator({
            name: 'IsExistTagDocumentDbValidate',
            target: object.constructor,
            propertyName: propertyName,
            options: validationOptions,
            validator: IsExistTagDocumentDbValidateConstraint,
        });
    };
}