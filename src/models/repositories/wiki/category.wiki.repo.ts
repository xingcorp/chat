...
@Injectable()
export class CategoryWikiRepo extends Repository<CategoryWiki> {
    constructor(private dataSource: DataSource) {
        super(CategoryWiki, dataSource.createEntityManager());
    }

    async createQueryBuilderWithCommonData(relations?: string[]): Promise<SelectQueryBuilder<CategoryWiki>> {
        const query = this
            .createQueryBuilder('qb')
            .leftJoinAndSelect(
                BRIDGE_TABLE_DB_OBJ.WIKI_CATEGORY_ORG_CHART.name,
                'wBridgeOrg',
                `"wBridgeOrg"."${BRIDGE_TABLE_DB_OBJ.WIKI_CATEGORY_ORG_CHART.joinColumn.name}" = qb.id`)
            .where(
                `"wBridgeOrg"."${BRIDGE_TABLE_DB_OBJ.WIKI_CATEGORY_ORG_CHART.inverseJoinColumn.name}" in (:...orgIds)`,
    ...
    private async filterQuery(query: SelectQueryBuilder<CategoryWiki>, filter: WikiCategoryFilterInput) {
        /*default get all*/
